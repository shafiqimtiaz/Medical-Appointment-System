...
userRouter.get('/manager/:id', async (req, res) => {
    try{
        ...
        if(manager){
            manager.user = await userService.findUserById(manager.user_id);
...
userRouter.get('/patient/:id', async (req, res) => {
    try{
        ...
        if(patient){
            patient.user = await userService.findUserById(patient.user_id);
...
userRouter.get('/staff/:id', async (req, res) => {
    try{
        ...
        if(staff){
            staff.user = await userService.findUserById(staff.user_id);
