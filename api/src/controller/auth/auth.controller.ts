    ...
    try{
      ...
      if(!existingUser){
        ...
        if(role == "manager"){
            ...
            const newPatients = await userService.createPatients(newUser.user_id, req.body);
            console.log(newPatients);
        }else if(role == "medical_staff"){
          const newMedical = await userService.createMedicalStaff(newUser.user_id, req.body);
